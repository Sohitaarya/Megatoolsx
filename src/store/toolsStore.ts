...
import { loadTools, getCategories, getToolsByCategory, findTool, searchCsvTools, AI_TOOLS, type CsvTool } from '@/data/csvData'
...
interface ToolsStore {
  ...
  // AI tools (separate collection, always available)
  ...
  init: async () => {
    ...
    try {
      ...
      set({
        ...
        latestTools: tools.slice(0, 24),
        popularTools: [...tools].sort(() => Math.random() - 0.5).slice(0, 24),
        newestTools: tools.slice(-24).reverse(),
        status: 'ready',
      })
    } catch (err) {
      set({ status: 'error', error: err instanceof Error ? err.message : 'Failed to load tools' })
    }
  },
